      ...
      <div className="mb-4 flex gap-4">
        {REPUTATION.map((r) => (
          <div key={r.dimension} className="flex flex-col">
            <span className="font-[family-name:var(--font-archivo)] text-3xl font-bold tabular-nums text-emerald-400">{r.value}</span>
      ...
      <div className="flex flex-wrap gap-1.5">
        {FIELDS.map((f) => (
          <button
            ...
            className={cn("rounded-md border px-2 py-1 font-mono text-sm", sel === f.key ? "border-emerald-500/50 bg-emerald-500/10 text-emerald-200" : "border-slate-700 text-slate-400 hover:text-slate-200")}
          >
            {f.key}
          </button>
        ))}
      </div>
      <p data-testid="field-explainer" className="mt-3 text-base text-slate-300">
