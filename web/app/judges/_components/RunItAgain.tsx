  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-5">
      <div className="flex items-center justify-between gap-4">
        ...
        <p data-testid="run-score" className="font-[family-name:var(--font-archivo)] text-6xl font-extrabold tabular-nums text-emerald-400">
          {audit.score}<span className="text-2xl text-slate-600">/100</span>
        </p>
      </div>

      <ul className="mt-4 space-y-1.5" aria-label="findings this run">
        {audit.findings.map((f) => (
          <li key={f.id} className="flex items-center gap-2 text-base motion-safe:animate-[fadein_240ms_ease]">
            <span className={cn("font-mono text-sm", f.severity === "critical" ? "text-red-400" : "text-slate-500")}>{f.severity}</span>
      ...
      <div className="mt-4 flex items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-emerald-500/90 px-4 py-2 text-base font-semibold text-slate-950 hover:bg-emerald-400"
        >
