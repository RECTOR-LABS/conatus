...
**Status: SHIPPED + WON.** 🏆 Grand Champion — Mantle Turing Test Hackathon 2026 + Best in Track (Dev Tool). Repo `main` = `2d729ae`, clean, in sync with `origin/main`. **138 tests passing** (67 web / 61 agent / 10 contracts). Site LIVE at https://conatus.rectorspace.com (`/judges` → 200). **$17,500 USDT prize: agreement SIGNED 2026-08-04 → $10 test transfer RECEIVED 2026-08-27** (Ethereum mainnet, native Tether — confirmed to Claire via email+TG same day) → **final payout targeted "next Monday" once all 30 teams confirm** (per Claire 2026-08-28). Chain note: test arrived on **Ethereum**, not Mantle (Exhibit A deviation, value-identical — RECTOR clarified with Claire directly). Tracker: `td-mrvnvj6c9acbwc`. LLM switched to **OpenRouter / `z-ai/glm-5.3-flash`** on 2026-08-04 (prod env swapped + redeployed).

**Prod infra (DO NOT rebuild — all verified LIVE):**
- Prod fully on **Vercel** (merged to `main` → auto-deploy). Prod LLM = **`z-ai/glm-5.3-flash` via OpenRouter** @ `https://openrouter.ai/api/v1` (switched 2026-08-04; before that `glm-5.2` via Ollama Cloud 2026-07-19→08-03; before that `anthropic/claude-sonnet-5` via OpenRouter). Dev LLM = same (local `.env`). Verified green via `E2E_DRY_RUN=1` e2e with real 5.3-flash synthesis; **live `report.model` = `z-ai/glm-5.3-flash` CONFIRMED on a real prod audit 2026-08-29** (job `a3fd568f`, Vault fixture, anchor:false — reentrancy high, LLM escalated setOwner to medium). **Cost model changed: Ollama = flat-rate; OpenRouter = pay-per-use.**
...
**Upstash Redis (job store + rate limit):** the free-tier marketplace store `conatus-verify-kv` was **auto-archived for inactivity** (~5 weeks idle post-Demo-Day) → DNS NXDOMAIN → `/api/audit` 500 across 2026-08-29 morning. Fixed same day: new free store **`upstash-kv-almond-door`** (iad1) provisioned via Vercel dashboard, dead `KV_*`/`REDIS_URL`/`UPSTASH_REDIS_REST_*` vars replaced, prod redeployed (`jbf20pxe4`), `/api/audit/<nil-uuid>` → 404, preflight 5/6 (only MNT-low). **Keep-alive: `.github/workflows/redis-keepalive.yml` probes weekly (Mon 09:17 UTC) — do NOT disable; a failed run = Redis down again.**

**Latest handoff (detailed):** `~/Documents/secret/strategy/conatus/session-handoff-2026-08-29.md` (prize signed + test transfer; LLM→OpenRouter/GLM-5.3-flash). Older handoffs (2026-08-04, 2026-07-19) + spec/plan docs in the same dir. Read the latest handoff first when resuming.
...
5. **Claim the $17,500 USDT Mantle prize** (in flight): DocuSign signed 2026-08-04 → **$10 test USDT received 2026-08-27** (Ethereum mainnet — confirmed same day) → final payout targeted "next Monday" once all 30 teams confirm (Claire, 08-28). Watch wallet `0x49d7…51d5` for ~$17,490 USDT + `rheza10@gmail.com` for any W-8BEN/KYC ask (**5-business-day** deadline, §2.3). After receipt: sweep to cold storage (wallet has only **~0.0007 ETH** gas on Ethereum — top up a little ETH first). Tracker: `td-mrvnvj6c9acbwc`.
...
- **SIGNED CONTRACT OBLIGATIONS (Hackathon Winner Award Agreement, signed 2026-08-04, Singapore law §13):** §10 — if Conatus ever does a **TGE**, the token MUST deploy on **Mantle Network** (multi-chain OK, Mantle required) — else material breach → **return the $17.5k Grant within 7 days** of notice. §14 — Mantle may terminate on **30 days' notice without cause** (or immediately for cause) → **return the Grant**. §9 — **24-month** good-faith Ecosystem Participation (showcases/meetups/demo days; reasonable notice + mutual availability). §5 — **all taxes on grantee** (W-8BEN likely requested). §2.3 — **5-business-day** deadline for verification-material requests, else forfeiture. Signed PDF in `~/Downloads`; full terms reviewed in the 2026-08-04 handoff. Prize flows to the personal EOA `0x49d7…51d5` (NOT the agent wallet). **Test transfer received 2026-08-27: $10 USDT on ETHEREUM mainnet** (native Tether; confirmed to Claire) — chain deviates from Exhibit A's "Mantle Network" (value-identical; clarified with Claire). Gas note for the post-payout sweep: the wallet holds only **~0.0007 ETH on Ethereum** — top up a little ETH before sweeping ~$17.5k to cold storage.
