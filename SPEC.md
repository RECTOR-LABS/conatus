...
- **🟡 IPFS pinning dependency.** Need a pinning provider (env-var key). *Mitigation:* Pinata (matches `agent0-sdk`'s default) or web3.storage; degrade gracefully to base64 `data:` URI on-chain if pinning fails (costs more DA but still verifiable).
