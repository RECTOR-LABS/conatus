...
import { simulateAttest, assertReceiptSuccess } from "../src/anchor";
...
describe("assertReceiptSuccess", () => {
  it("accepts a successful receipt", () => {
    expect(() => assertReceiptSuccess({ status: "success" }, "https://example/tx/0xabc")).not.toThrow();
  });

  it("throws an actionable error naming the tx when the receipt is reverted", () => {
    expect(() => assertReceiptSuccess({ status: "reverted" }, "https://example/tx/0xabc")).toThrow(
      /reverted.*https:\/\/example\/tx\/0xabc/s,
    );
  });
});
